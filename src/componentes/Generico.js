...
class Generico extends Component{
    ...
    onCambiarVentana = async idVentanaSeleccionada =>{
        ...
        if(idVentanaSeleccionada === 4){
          ...
          result = Array.from(new Set(datosEG.datos.map(s=> s.Tarea.TxtCluster)))
            ...
            this.setState({idVentana: idVentanaSeleccionada, clustersVentana: result, datosVentanaEG: datosEG});
        }else{
          ...
          result = result.filter(x=> x.cluster !== undefined);
          this.setState({idVentana: idVentanaSeleccionada, clustersVentana: result, datosVentana: datosActs, totalAdmin: ventanas[0].Administración.length, totalNorm: ventanas[0].Normativo.length, totalProy: ventanas[0].Proyectos.length});
        }
    }
    ...
    render(){
                    ...
                    if(fila.Tarea.OrdenEG === props.idCluster){
                        ...
                        return (
                            <div key ={fila.ID} className="row item">
                                ...
                                <Columna titulo= {<p><img title={fila.AsignadoA !== undefined ? fila.AsignadoA : 'Sin asignar'} src= {fila.AsignadoA !== undefined ? assignedTo_icon : plus_icon} alt='assignedTo_icon' onClick={()=>{ this.onAbrirModal(this.props.terreno, 270, false )} } /></p> } estilo = 'col-sm' editable= {false} />
                            </div>
                        )
                    }else{
                        return null
                    ...
                    if(fila.IdTarea.Orden === props.idCluster){
                        return (
                            <div key ={fila.ID} className="row item">
                                ...
                                <Columna titulo= {<p><img title={fila.AsignadoA !== undefined ? fila.AsignadoA : 'Sin asignar'} src= {fila.AsignadoA !== undefined ? assignedTo_icon : plus_icon} alt='assignedTo_icon' onClick={()=>{ this.onAbrirModal(this.props.terreno, 270, false )} } /></p> } estilo = 'col-sm-1' editable= {false} />
        ...
        const {idVentana, totalAdmin, totalNorm, totalProy} = this.state
        return(
            <div>
                <Backdrop abierto= {!this.state.cargado} />
                <Encabezado terreno = {this.props.terreno} idVentana = {this.state.ventana} cambiarVentana={this.onCambiarVentana} totalAdmin = {totalAdmin} totalNorm = {totalNorm} totalProy = {totalProy}  />
                {this.state.cargado ?
