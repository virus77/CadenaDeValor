...
class Encabezado extends Component {
    ...
    onCambiarVentana = (idVentana, mensaje, name, style) => {
        if (idVentana !== this.props.idVentana) {
            this.props.cambiarVentana(idVentana, mensaje, name, style, '', '')
        }
    }
            ...
            <div>
                <div className='container-fluid' style={{ paddingTop: "1%" }}>
                    <div className='row'>
                        ...
                        <div className='col-sm-1 columna'><img id='FiltroFavoritos' onClick={() => this.onCambiarVentana(5, disabled)} src={idVentana !== 4 ? fav_icon : favDis_icon} alt='favoritos_icon' disabled={disabled} ></img></div>
                        <div className='col-sm-1 columna'><img id='FiltroGantt' onClick={() => this.onCambiarVentana(6, false)} src={idVentana !== 4 ? gantt_icon : gantt_icon} alt='gantt_icon' disabled={false} ></img></div>
                        <div className='col-sm-1 columna'><img id='FiltroVerTodo' onClick={() => this.onCambiarVentana(7, disabled)} src={idVentana !== 4 ? viewAll : viewAllDis} alt='user_icon' disabled={disabled} ></img></div>
                        <div className='col-sm-1 columna'><img id='MACO' src={this.props.maco === '' || this.props.maco === null || this.props.maco === undefined ? macox : (this.props.maco === 'B' ? macob : macoc)} alt='macob' onClick={() => { this.onAbrirModal(terreno, 268, false, 'radioChecked', maco, { Tarea: { ID: 268 } }) }} ></img></div>
                        ...
                        <div className='col-sm-5 menu'>
                            <nav className="navbar navbar-expand-sm bg-light navbar-light">
                                <ul className="navbar-nav" style={{ borderBottomStyle: "solid", borderBottomColor: "#3C8891" }}>
                                    <li name='admin' className={idVentana === 1 ? "nav-item active colorBlueMenu" : "bg-light nav-item colorNoMenu"} onClick={(e) => this.onCambiarVentana(1, 'Cargando contenido...', "genericoAdmin.css", "../estilos/genericoAdmin.css")}>
                                    ...
                                    <li name='norm' className={idVentana === 2 ? "nav-item active colorBlueMenu" : "bg-light nav-item colorNoMenu"} onClick={(e) => this.onCambiarVentana(2, 'Cargando contenido...', "genericoNorm.css", "../estilos/genericoNorm.css")}>
                                        <a className={idVentana === 2 ? "nav-link colorWhite" : "nav-link disabled"} href="#">Normativo <Badge color="secondary">{totalNorm}</Badge></a>
                                    </li>
                                    <li name='proy' className={idVentana === 3 ? "nav-item active colorBlueMenu" : "bg-light nav-item colorNoMenu"} onClick={(e) => this.onCambiarVentana(3, 'Cargando contenido...', "genericoProy.css", "../estilos/genericoProy.css")}>
                                        <a className={idVentana === 3 ? "nav-link colorWhite" : "nav-link disabled"} href="#">Proyectos <Badge color="secondary">{totalProy}</Badge></a>
                                    </li>
                                    <li name='eg' className={idVentana === 4 ? "nav-item active colorBlueMenu" : "bg-light nav-item colorNoMenu"} onClick={(e) => this.onCambiarVentana(4, 'Cargando contenido...', "genericoEG.css", "../estilos/genericoEG.css")} >
