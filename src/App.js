...
class App extends Component {
  ...
  onSeleccionTerreno = async (IdTerreno, IdProyecto, TxtTerreno) => {
    ...
    if (window.confirm('¿Está seguro que desea abrir el detalle del terreno ' + TxtTerreno + "?")){
      ...
      this.setState({isInActive: false, isActive: true, idTerreno: IdTerreno, idProyecto: IdProyecto, nombreTerreno: TxtTerreno });
    }
    else{
      this.setState({ isActive: false, idTerreno: IdTerreno, idProyecto: IdProyecto, nombreTerreno: '' })
    }
  }
  ...
  render(){
    const { itemsT, itemsPI, idProyecto, idTerreno, nombreTerreno, modal, veg, v, contadorAdministracion, contadorNormativo, contadorProyectos } = this.state;
    return (
      <div className="App">
        {this.state.isInActive ? <Principal selecciontereno={this.onSeleccionTerreno} itemsT={itemsT} itemsPI={itemsPI} /> : null}
        {/*this.state.isActive ? <Encabezado terreno = {nombreTerreno} idVentana = {this.state.ventana} abrirModal={this.onOpenModal} cambiarVentana={this.onSelectWindow} totalAdmin = {contadorAdministracion} totalNorm = {contadorNormativo} totalProy = {contadorProyectos}  /> : null*/}
        {/*this.state.isActive ? <Generico idProyecto = {idProyecto} idTerreno = {idTerreno} terreno = {nombreTerreno} clusters = {this.state.clustersVentana} datosVentana = {this.state.ventana === 4 ? veg[0] : v[0]} idVentana = {this.state.ventana} abrirModal={this.onOpenModal} /> : null*/}
        {this.state.isActive ? <Generico idProyecto = {idProyecto} idTerreno = {idTerreno} terreno = {nombreTerreno} idVentana = {this.state.ventana} /> : null}
